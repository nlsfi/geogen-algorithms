...
def generalize_road_links(
    ...
    # Step 1
    road_links_1 = check_line_connections_original(
        road_links,
        threshold_distance
        )
    ...
    # Step 2
    inspected_candidates = check_line_connections_multilayer(
    ...
    removed_short_unconnected = inspected_candidates[(~inspected_candidates['is_connected']) & (inspected_candidates['geometry'].length > threshold_length)]
    ...
    roads_to_keep = road_links_4[(road_links_4['dead_end'] == False) | (road_links_4['dead_end_connects_to_path'] == True)]  # noqa
    roads_to_generalize = road_links_4[(road_links_4['dead_end'] == True) & (road_links_4['dead_end_connects_to_path'] == False)]  # noqa

    remove_short_dead_ends = roads_to_generalize[roads_to_generalize['geometry'].length > threshold_length]
    ...
    shared_paths_generalization_output = (
        roadside_shared_paths[~roadside_shared_paths['roadside']]
        )
...
def check_line_connections_multilayer(
...
def detect_shared_paths_along_roads(
        gdf_shared_path: gpd.GeoDataFrame,
    ...
    for idx, row in gdf_shared_path.iterrows():
        line = row.geometry
