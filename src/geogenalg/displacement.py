...
def displace_points(
    input_gdf: gpd.GeoDataFrame, displace_threshold: float
) -> gpd.GeoDataFrame:
    """Move points apart so that no two points are closer than a threshold.

    Note: If there are more than two points located close to each other, this
    function may need to be called iteratively to ensure that all point-to-point
    distances exceed the threshold.

    Args:
    ----
        input_gdf: A GeoDataFrame with point geometries
        displace_threshold: Minimum allowed distance between points
    ...
    original_coordinates = np.array([[geom.x, geom.y] for geom in result_gdf.geometry])
